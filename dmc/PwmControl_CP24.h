...
typedef enum{
    ...
    PwmErr=1,                      // Ошибка модуля ШИМ
    PwmInitFeedTopKey=2,           // Настройка модуля Шим к запитке драйвера верхних ключей  
    PwmFeedTopKey=3,               // Запитка драйвера верхних ключей и датчиков тока
    PwmCalibrOffset=4,             // Калибровка каналов измерения (определение Оффсет)
    PwmInitCompMtz=5,              // Настройка компаратора ацп для контроля МТЗ 
    prePwmStart=6,                 // Второй такт калибровки нулей АЦП ДТ
    PwmStart=7,                    // Запуск ШИМ 
    PwmWork=8,                     // Штатная работа модуля ШИМ (ШИМ включен!!!) 
