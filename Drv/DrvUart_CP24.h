...
#define WIFI_PROG_ON_Pin     GPIO_PIN_14
#define WIFI_PROG_ON_Port    GPIOB

#define RS485_DIR_Pin        GPIO_PIN_11
#define RS485_DIR_GPIO_Port  GPIOA
// Значения направления передачи сделаны глобальными переменными с теми же названиями, для удобства работы.
// Их определение происходит в RS485_GPIO_Init() в зависимости от типа блока Ср24 
//#define RS485_TX_ENABLE      GPIO_PIN_SET  	// Указываем состояние ноги направления - на передачу
//#define RS485_RX_ENABLE      GPIO_PIN_RESET 	// Указываем состояние ноги направления - на прием

#define PULT_DIR_Pin         GPIO_PIN_10
#define PULT_DIR_GPIO_Port   GPIOB
...
typedef struct {
  ...
  u16   status;                      // Переменная статуса для WIFI модуля
  u16   needModbusSlave;            // Флаг, что UARTу требуется обращение в задачу Modbus Slave
  u32   *pntUart;          //указатель на физически юарт - заглушка для VS2005
}uart_type;
#endif
