...
u16 fileRd(unsigned char *Bff, u16 file, u32 adr, u16 size)//SEM_Obj
{
    ...
    {
      case FILE_In_Flash:
          // Если семафор свободен, перед началом операции его на до поднять.
          if (osSemaphoreGetCount (FlashSemRdWr))      osSemaphoreAcquire(FlashSemRdWr, portMAX_DELAY);        
          ...
          osSemaphoreRelease(FlashSemRdWr);                              // После ответа отдать семафор
      break;
    ...
    if(pmsg->codeFail)                                                  // если возникла ошибка при работе с Flash паматью
        return ErrMb(Bff, SLAVE_DEVICE_BUSY_merr);                      // возможно стоит изменить код ошибки
    else if((*msg.opCode & ERRat45sign) != 0 )
