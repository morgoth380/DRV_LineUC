...
// Inverse park transformation on the rotor flux from the current model
    v->FluxDrS = v->FluxDrE * arm_cos_f32(v->ThetaFlux);
    v->FluxQrS = v->FluxDrE * arm_sin_f32(v->ThetaFlux);
...
// Compute the rotor flux angle
    ...
    switch (v->FluxThetaCorMode) {
        ...
        case FluxThetaCorModePID:
          ...
          // Заменить на модели статора !!!!
          
          // PsiErr = v->PhiR_nom - PsiR;
          
          if (fabsf(v->Fcur) > (2.0f/80.0f) && v->GenMode ) {            
            v->PsiErrProp = 0; // Обнулить пропорционал до начала расчета
            // Если поток снизился, начинаем корректировать
            if (v->PsiR <= v->FluxDrE) {
              // пропорц часть
              v->PsiErrProp = PsiErr * v->FluxThetaPidKp;
              
              // Интегральная часть всегда накапливается, а когда вышел из зоны, то сдвиг остается
              v->PsiErrInteg = v->PsiErrInteg + (PsiErr * v->Tpwm) * v->FluxThetaPidKi;
            } 
            
            dTheta = v->PsiErrProp + v->PsiErrInteg;            
            // Сдвиг фазы только увеличиваем, вниз не опускаем. 
            if (dTheta > v->dTheta) {
                //v->dTheta = dTheta;
                v->dTheta = f32_Filtr(v->dTheta, dTheta, 0.03125f);             
            }
              
              //v->dTheta = v->PsiErrProp + v->PsiErrInteg;            
            
            
          } else { // Только когда отключаем генераторный режим начинаем уменьшать сдвиг фазы.
            v->dTheta = f32_Filtr(v->dTheta, 0.00f, 0.0078125f); // за 256 отсчета снижаем фазу            
          }
        break;            
    }  
    
    // корректировка угла потокосцепления ротора
    if (v->Fcur >= 0) { // Если текущая частота выше нуля, то фазу прибавляем
        v->ThetaFlux = v->ThetaFlux + v->dTheta;
    } else { // Иначе при отрицательной частоте, фазу надо отнимать, для получения опрежения.
        v->ThetaFlux = v->ThetaFlux - v->dTheta;
    }
